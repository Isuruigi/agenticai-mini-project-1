"""
Q&A Generation Module for Operation Ledger-Mind (Gemini Version)
Generates question-answer pairs from document chunks using Google Gemini API
"""
...
def generate_questions(chunk: str, model, max_retries: int = 3) -> List[str]:
    """
    Generate 10 questions from a chunk using Gemini
    ...
    for attempt in range(max_retries):
        try:
            response = model.generate_content(
                QUESTION_GENERATION_PROMPT.format(chunk=chunk),
                generation_config={
...
def generate_answer(chunk: str, question: str, model, max_retries: int = 3) -> str:
    """
...
def create_qa_dataset(
    ...
    """
    Create Q&A dataset from chunks using Gemini
    ...
    model = genai.GenerativeModel('gemini-2.5-flash')  # Confirmed working!
    ...
    print(f"\n{'='*60}")
    print(f"Q&A GENERATION (Using Google Gemini)")
    print(f"{'='*60}")
    ...
    for idx, chunk in enumerate(tqdm(chunks, desc="Generating Q&A pairs")):
        try:
            # Generate questions
            questions = generate_questions(chunk, model)
            ...
            for question in questions:
                try:
                    answer = generate_answer(chunk, question, model)
            ...
            time.sleep(1)  # 60 requests per minute limit
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Q&A GENERATION TEST (Gemini)")
    print("=" * 60)
